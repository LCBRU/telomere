...
def speadsheet_upload():
    ...
    form.batch.operatorUserId.choices = [
        (u.id, u.GetCodeAndName()) for u in users
    ]

    form.batch.operatorUserId.data = (
        form.batch.operatorUserId.data or current_user.id
    )
    ...
    if form.validate_on_submit():
        ...
        if batchService.IsBatchDuplicate(batch):
            if batch.is_initial() or batch.is_replate():
                flash(
                    "The plate name and half plate have "
                    "been used in a previous batch.",
                    "error"
                )
                db.session.rollback()
            ...
            if batch.is_duplicate():
                flash(
                    "The plate name and half plate have not been "
                    "used in a previous batch.",
                    "error"
                )
                db.session.rollback()
                return render_template('spreadsheet/upload.html', form=form)
        ...
        if (batch):
            ...
            spreadsheet = spreadsheetService.SaveAndReturn(
                form.spreadsheet.data, batch
            )
            ...
            spreadsheetLoadResult = spreadsheetService.Process(
                spreadsheet,
                not batch.is_replate()
            )
            ...
            if spreadsheetLoadResult.abortUpload():
                if len(spreadsheetLoadResult.missingSampleCodes) > 0:
                    flash(
                        "The following samples are not in the manifest: %s" %
                        ", ".join(
                            str(x) for x in
                            spreadsheetLoadResult.missingSampleCodes
                        ),
                        "error")
                if len(spreadsheetLoadResult.incorrectPlateName) > 0:
                    flash(
                        "The following samples have a different plate name "
                        "to the manifest: %s" % ", ".join(
                            str(x) for x in
                            spreadsheetLoadResult.incorrectPlateName
                        ),
                        "error"
                    )

                flash(
                    "File '%s' has not been uploaded" % spreadsheet.filename,
                    "error"
                )
